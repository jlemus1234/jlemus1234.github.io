...
- Next: step 13, cutover — **explicit go-ahead required before the force-push**. Steps: re-verify archive repo is current, reset `master` to this branch's tree as fresh orphan history + force-push, flip Settings → Pages → Source to "GitHub Actions", verify the live site end-to-end (including that the placeholder video shows on all 5 project pages until step 7 happens — expected, not a bug). This is the last step in the whole plan.
